    ...
    public void storeEmploye(Employe employe) {
        jdbcTemplate.update(connection -> {
            final PreparedStatement preparedStatement = connection
                    .prepareStatement("INSERT INTO EMPLOYES (ID_NUMBER, ID_TYPE, NAME, LAST_NAME) VALUES (?, ?, ?, ?)");

            preparedStatement.setString(1, employe.getIdentificationNumber().getValue());
            preparedStatement.setString(2, employe.getIdentificationType().name());
            preparedStatement.setString(3, employe.getName().getValue());
            preparedStatement.setString(4, employe.getLastName().getValue());
